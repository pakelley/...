...
(after! notmuch
  (setq notmuch-saved-searches '((:name "pm/imbox"            :query "tag:protonmail AND tag:imbox AND NOT tag:trash AND NOT tag:deleted"       :key "u")
                                 ...
                                 (:name "htx/imbox"           :query "tag:heartex AND tag:imbox AND NOT tag:trash AND NOT tag:deleted"          :key "n")
                                 (:name "htx/feed"            :query "tag:heartex AND tag:feed AND NOT tag:trash AND NOT tag:deleted"           :key "m" :search-type 'tree)
                                 (:name "htx/paper-trail"     :query "tag:heartex AND tag:paper-trail AND NOT tag:trash AND NOT tag:deleted"    :key ",")
                                 (:name "htx/screener"        :query "tag:heartex AND tag:screener AND NOT tag:trash AND NOT tag:deleted"       :key "."))))
