...
Viewing funtions:
#+name: view-opened-date-orgql
#+begin_src emacs-lisp :tangle no
(defun +patch/get-task-file (task)
  "Get the file that a task is in. Important for retrieving the content of a task. This function is definitely specific to my own task organization."
  (let ((archive-file (org-element-property :ARCHIVE_FILE task)))
    (or archive-file "~/.local/share/notes/gtd/org-gtd-tasks.org")))
...
(defun +patch/get-contents (task)
  "Using the org element API, get the contents of a task (i.e. the plain text after the headline).

If there's a marker, use that (because it's more robust), otherwise recursively search up the task tree for a ':path' property (and hope for the best)."
  (with-current-buffer (let ((marker (org-element-property :org-marker task)))
                         (if marker (marker-buffer marker)
...
(defconst +patch/old-org-ql-opened-regexp
  (rx bol "- State \"" (1+ letter) "\"" (1+ blank) "from \"READY\"" (1+ blank) (group (1+ not-newline))))

(defconst +patch/org-ql-opened-regexp
  (rx bol ":OPENED:   " (group (1+ not-newline))))

(defun +patch/get-opened-date (task)
  "Get the date a task was opened (i.e. moved from READY to TODO/NEXT) using the org element api. Requires fetching the content of the task (which I don't have a reliable process for yet)."
...
(defun +patch/set-opened-date-after-state-change ()
  (when (equal org-last-state "READY")
    (+patch/set-opened-date (point)
                            (format-time-string
                             "%Y-%m-%d"
                             org-log-note-effective-time))))
(add-hook 'org-after-todo-state-change-hook '+patch/set-opened-date-after-state-change)
    #+end_src
Predicate functions:
#+name: planning-predicate-orgql
#+begin_src emacs-lisp :tangle no
...
#+begin_src jupyter-python :session burnup :var burnup_data=quarters-tasks :tangle no
import pandas as pd
import numpy as np
import matplotlib
