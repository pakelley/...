...
#+begin_src emacs-lisp :tangle no
(jiralib-call "getIssuesFromJqlSearch" org-jira-get-issue-list-callback org-jira-default-jql 100)
(jiralib--rest-call-it
 "/rest/api/3/search/jql"
 :type "GET"
 :params `((jql . "project = \"DSML\" AND Sprint in openSprints() AND assignee = currentUser()")
           (maxResults . 10)
           (fields . "*all")
           (expand . "renderedFields")))

(patch/jiralib--rest-call-it
 "https://bombora.atlassian.net/rest/api/3/search/jql"
 :type "GET"
 :params `((jql . "project = \"DSML\" AND Sprint in openSprints() AND assignee = currentUser()")
           (maxResults . 10)
           (fields . "*all")
...
(patch/jiralib--rest-call-it
 "https://bombora.atlassian.net/rest/api/3/issue/DSML-8852"
 :type "GET")

(defun patch/jiralib--rest-call-it (api &rest args)
...
* Task Planning
** Task Queue System
The task queue system manages task prioritization across four levels (coldest to hottest):
- *Icebox* - Someday/maybe ideas (TODO, @@icebox)
- *Shelf* - Selected for this year, not yet in quarterly planning (TODO, @@shelf)
- *Backburner* - Pulled into this quarter (TODO, @@backburner, OPENED)
- *Frontburner* - This week's priorities (TODO/NEXT, @@frontburner, OPENED)

Core functions are tangled to =dayone-task-queue.el= for independent testing.

*** Running Tests
From the =org-patch= directory:
#+begin_src shell :tangle no
emacs --batch \
  -L . \
  -L ~/.config/doom-emacs/.local/straight/build-30.1/buttercup \
  -L ~/.config/doom-emacs/.local/straight/build-30.1/ts \
  -L ~/.config/doom-emacs/.local/straight/build-30.1/dash \
  -L ~/.config/doom-emacs/.local/straight/build-30.1/s \
  -l buttercup \
  -l dayone-task-queue \
  -l test/test-task-queue.el \
  -f buttercup-run
#+end_src

*** Library Header
#+begin_src emacs-lisp :tangle dayone-task-queue.el
...
*** Planning View Variants
#+name: queue-planning-functions
#+begin_src emacs-lisp :tangle dayone-task-queue.el
...
*** Bulk Actions and Keybindings
#+name: queue-keybindings
#+begin_src emacs-lisp :tangle no
...
*** Org-QL Queue Predicates
#+name: queue-predicates
#+begin_src emacs-lisp :tangle dayone-task-queue.el
...
Quarterly planning is done with an org-ql view of my "Cold Tasks (Shelf, Icebox)" on the left, and the "Backburner" on the right. The process is to:
1. All Wishes Granted: Pull promising tasks from the left (Cold Tasks) to the right (Backburner) to get an "all wishes granted" backburner queue
...
* Task Archiving
Keep my GTD file slim by archiving completed work into a dedicated archive file while keeping the original outline structure intact. Each archived subtree lands under the same parent headings it had in the source file, so projects stay grouped together. Holding =DEL a= still archives the subtree at point, while =DEL A= sweeps an entire buffer (defaults to my =org-gtd= tasks file) for =DONE= and =CNCL= items and archives them in one go.

Set these vars to tweak the behaviour:
- `+patch/org-archive-file-name`: base name for the archive file
- `+patch/org-archive-file-extension`: optional extension (defaults to no extension to match existing files)
- `+patch/org-archive-directory`: where archive files are stored, defaults to the GTD directory

I also added utilities to migrate the old datetree archive into the new outline-preserving file:
- `+patch-gtd/migrate-legacy-archive-entry`: move the entry at point using its =ARCHIVE_OLPATH= metadata
- `+patch-gtd/migrate-legacy-archive`: migrate every archived entry (i.e. nodes with =ARCHIVE_TIME=) in the current subtree (use =C-u= to operate on the whole buffer)
Both commands fill in missing =ARCHIVE_OLPATH= data by applying the same metadata helper I use for normal archiving: they look at the current outline path (so you can reorganize entries first) and record it before moving anything.
...
#+end_src
* GTD
I'm largely not using this anymore, still working on the final transition to the "dayone" system over this.
** Keybindings
Global keybindings:
| Keybinding | Description                    |
|------------+--------------------------------|
